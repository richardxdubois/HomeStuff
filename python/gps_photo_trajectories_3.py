...
from bokeh.models import (ColumnDataSource, Slider, Button, Toggle, CustomJS, Div, Legend, Span, HoverTool, Range1d,
                          WMTSTileSource)
...
from correlate_photo_gps import photo


class gps_photo_trajectories():
    """
    ...
    trajectories. Javascript updates those 2 elements via a source.change.emit.

    Optionally add locations of photos along the route.
    ...
    def __init__(self, config=None):
        ...
        self.html_name = self.html_dir + self.title + ".html"
        ...
        del_div, t_hist = self.create_figure()
        ...
        if self.data_dir is not None:
            ...
            self.refresh_slider = Slider(start=self.slider_start, end=self.slider_end, value=self.refresh_rate,
                                         step=step,
            ...
            d_hist, e_hist, legend = self.setup_gpx_figures(t_hist=t_hist)
            ...
            if self.photo_dir is not None:
                rc = self.setup_photos(t_hist=t_hist)

            # Layout the application
            if self.data_dir is not None:
                canvas_layout = column(del_div, row(self.toggle, self.refresh_slider),
                                       row(d_hist, column(t_hist, e_hist)))
            else:
                canvas_layout = column(del_div, t_hist)
    ...
    def setup_photos(self, t_hist=None):
        ...
        for p in ph:
            ...
            if img.latitude is not None:
                ...
                for g in self.file_stems:
                    ...
                    t_diff = []
                    for i in range(len(self.time[g])):
                        t_diff.append((self.time[g][i] - time_tz).seconds)
            ...
            if p_m_x != -999:
                ...
                p_merc_y.append(p_m_y)
                name = str(Path(p).name)
                caption = img.camera + ": " + str(Path(p).name)
                if img.caption is not None:
                    caption += " - " + img.caption
                p_caption.append(caption)

                # p_name.append(name)

                if "file://" in self.url_prefix:
                    url = self.url_prefix + p
                else:
                    url = self.url_prefix + name

                p_url.append(url)

        print("skipped ", self.no_gps, " gps files")
        cds = ColumnDataSource(dict({"lat": p_merc_x, "lon": p_merc_y, "index": p_caption, "image_url": p_url}))
        ...
        buf_x = 0.05 * (max_x - min_x)
        buf_y = 0.05 * (max_y - min_y)

        t_hist.x_range = Range1d(min_x - buf_x, max_x + buf_x)
        t_hist.y_range = Range1d(min_y - buf_y, max_y + buf_y)

        t_hist.scatter(x="lat", y="lon", source=cds, color='red')
    ...
    def list_files_in_folder(self, folder):
        ...
        files = [str(p) for p in p_folder.iterdir() if p.is_file()]
        return files
    ...
    def create_figure(self):
        ...
        if self.map == "NatGeoWorldMap":
            t_hist.add_tile(getattr(xyz.Esri, self.map))
        elif self.map == "EsriWI":
            imagery = WMTSTileSource(
                url="https://services.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{Z}/{Y}/{X}.jpg",
                attribution="Esri World Imagery"
            )
            labels = WMTSTileSource(
                url="https://services.arcgisonline.com/ArcGIS/rest/services/Reference/"
                    "World_Boundaries_and_Places/MapServer/tile/{Z}/{Y}/{X}.png",
                attribution="Esri World Boundaries & Places (labels)"
            )

            t_hist.add_tile(imagery)
            t_hist.add_tile(labels)  # draw labels on top

        hover_wt = HoverTool(tooltips=self.thumb_div)
        t_hist.add_tools(hover_wt)

        div_text = (self.title + " Run on: " + datetime.now().strftime("%Y-%m-%d %H:%M:%S") +
                    "<br> Does NOT work on mobile browsers"
                    "<br> Interactive map: use pan, zoom and reset icons on right hand side under Bokeh icon"
                    "<br> click on map - 2-finger drag for zoom; 3-finger drag for pan. then hover over red markers"
                    " to see photos"
                    "<br> Missing " + str(self.no_gps) + " gps files")
        ...
        for track in gpx.tracks:
            for segment in track.segments:
                ...
                for ip, point in enumerate(segment.points):
                    ...
                    self.time.setdefault(self.current_file_stem, [])
                    self.time[self.current_file_stem].append(time_tz)
    ...
    def setup_gpx_figures(self, t_hist=None):
        """
        ...
        for i, k in enumerate(self.file_stems):
            ...
            if self.full_trajectories:
                t_hist.x_range = Range1d(x_min, x_max)
                t_hist.y_range = Range1d(y_min, y_max)
...
if __name__ == "__main__":
    ...
    a = gps_photo_trajectories(config=args.app_config)
