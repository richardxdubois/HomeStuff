...
from bokeh.models import (ColumnDataSource, HoverTool, Span, Label,
                          Div, DatetimeTickFormatter, Slider, CustomJS,
                          Range1d)
...
class DiskDashboard:
    ...
    def generate(self):
        ...
        table_cutoff = datetime.now() - timedelta(days=self.table_days)
        table_df = df[df['timestamp'] >= table_cutoff]

        generated = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        date_range = (f"{df['timestamp'].min():%Y-%m-%d} to "
                      f"{df['timestamp'].max():%Y-%m-%d}")

        # Build components
        status_cards = self._build_status_cards(df, disks, colors)
        free_chart, pct_chart, slider = self._build_linked_charts(
            df, disks, colors, total_days
        )
        alert_table = self._build_alert_table(table_df)
        recent_table = self._build_recent_table(table_df)
        ...
        header = Div(text=f"""
            ...
            <div class="subtitle">
                Generated: {generated} ·
                Full history: {date_range} ({total_days} days) ·
                Table: last {self.table_days} days ·
                Log: {self.log_file}
            </div>
        """, sizing_mode='stretch_width')

        cards_div = Div(text=status_cards, sizing_mode='stretch_width')
        slider_label = Div(
            text='<div class="slider-label">'
                 '📅 Adjust plot time window (days):</div>',
            sizing_mode='stretch_width'
        )
        free_title = Div(
        ...
        pct_title = Div(
            text='<div class="section-title">'
                 'Used Percentage Over Time</div>',
            sizing_mode='stretch_width'
        )
        alert_div = Div(text=alert_table, sizing_mode='stretch_width')
        recent_div = Div(text=recent_table, sizing_mode='stretch_width')
    ...
    def _build_linked_charts(self, df, disks, colors, total_days):
        """Build free space and used % charts linked to a time slider."""

        fig_free = self._styled_figure(y_label="Free Space (GB)")
        fig_pct = self._styled_figure(y_label="Used (%)")
        fig_pct.y_range = Range1d(0, 100)
        ...
        for disk in disks:
            ...
            # Free space chart
            fig_free.line('timestamp', 'free_gb', source=source,
                          line_width=2, color=color, legend_label=disk)
            fig_free.scatter('timestamp', 'free_gb', source=source,
            ...
            fig_free.add_layout(threshold_span)
            fig_free.add_layout(Label(
                x=10, y=threshold, x_units='screen',
                text=f'{disk} threshold ({threshold:.0f} GB)',
                text_color=color, text_font_size='10px',
                text_alpha=0.7, y_offset=5
            ))

            # Used % chart
            fig_pct.line('timestamp', 'used_pct', source=source,
                         line_width=2, color=color, legend_label=disk)
            fig_pct.scatter('timestamp', 'used_pct', source=source,
                            size=5, color=color, alpha=0.7,
                            legend_label=disk)

        # Warning/critical lines on % chart
        fig_pct.add_layout(Span(
            location=90, dimension='width',
            line_color='orange', line_dash='dashed', line_width=1
        ))
        fig_pct.add_layout(Label(
            x=10, y=90, x_units='screen',
            text='90% Warning', text_color='orange',
            text_font_size='10px', y_offset=5
        ))
        fig_pct.add_layout(Span(
            location=95, dimension='width',
        ...
        fig_free.add_tools(hover_free)
        fig_pct.add_tools(hover_pct)
        ...
        callback = CustomJS(
            ...
            code="""
                const days = cb_obj.value;
                const ms_per_day = 86400000;
                const end = t_max_ms + ms_per_day * 0.05;
                const start = t_max_ms - (days * ms_per_day);

                fig_free.x_range.start = start;
                fig_free.x_range.end = end;
                fig_pct.x_range.start = start;
                fig_pct.x_range.end = end;
            """
        )
        slider.js_on_change('value', callback)

        return fig_free, fig_pct, slider
    ...
    def _build_alert_table(self, df):
        ...
        if alerts.empty:
            return (f'<div class="section-title">'
                    f'⚠️ Alert History (last {self.table_days} days)</div>'
                    f'<p class="success">✅ No alerts in this period.</p>')
        ...
        <div class="section-title">
            ⚠️ Alert History — last {self.table_days} days
            ({len(alerts)} events)
        </div>
        <table>
            <tr>
    ...
    def _build_recent_table(self, df):
        ...
        recent = df.sort_values('timestamp', ascending=False).head(200)
        ...
        return f"""
        <div class="section-title">
            Recent Readings — last {self.table_days} days
        </div>
        <table>
            <tr>
                <th>Time</th><th>Disk</th><th>Status</th>
