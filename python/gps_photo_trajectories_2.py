...
from correlate_photo_gps import correlate_photo_gps, photo
...
class gps_photo_trajectories():
    ...
    def __init__(self, config=None):
        ...
        self.correlate_photos_yaml = data["correlate_photos_yaml"]
        self.correlated_photos = None

        if self.data_dir is not None:
            p = correlate_photo_gps(config=self.correlate_photos_yaml)
            self.correlated_photos = p.correlate_gpx_photos()
        ...
        # Create a slider for refresh rate
            ...
            div_text, anim, d_hist, e_hist, legend = self.setup_gpx_figures(name=self.file_stems[0])
            ...
            div_text, anim = self.setup_photos_only()

            # Layout the application
            canvas_layout = column(div_text, anim)

        output_file(self.html_name)

        save(canvas_layout, title="Animate GPS Multi Trajectories")

    def setup_photos_only(self):
        ...
        cds = ColumnDataSource(dict({"lat": p_merc_x, "lon": p_merc_y, "index": p_name, "image_url": p_url}))
    ...
    def create_figure(self):
        ...
        del_div = Div(text=self.title + " Run on: " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    ...
    def setup_gpx_figures(self, name=None):
        """
        ...
        for i, k in enumerate(self.file_stems):
            ...
            if self.correlate_photos_yaml is not None:
                try:
                    ph = self.correlated_photos[k + ".gpx"]
                    p_merc_x = []
                    p_merc_y = []
                    p_name = []
                    p_url = []
                    ...
                    for p in ph:
                        p_m_x, p_m_y = self.latlon_to_mercator(lat=ph[p]["lat"], lon=ph[p]["lon"])
                        p_merc_x.append(p_m_x)
                        p_merc_y.append(p_m_y)
                        p_name.append(p)
                        if "file://" in self.url_prefix:
                            url = self.url_prefix + ph[p]["filespec"]
                        else:
                            url = self.url_prefix + p
                    ...
                    t_hist.x_range = Range1d(min(min(p_merc_x), x_min), max(max(p_merc_x), x_max))
                    t_hist.y_range = Range1d(min(min(p_merc_y), y_min), max(max(p_merc_y), y_max))
                    t_hist.scatter(x="lat", y="lon", source=cds, color='red')
